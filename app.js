...
migrateReturnVat();
function recordInventoryChange(product, change, reason, reference = '') { state.inventoryHistory.unshift({ product: product.name, sku: product.id, change, reason, reference, date: 'Sep 23, 2026' }); }
...
function renderOverview() { return `<div class="page-heading"><div><div class="eyebrow">Tuesday, September 23, 2026</div><h1>Good morning, Alex</h1><p>Here’s what’s happening across Atelier Market today.</p></div><button class="date-chip">This month　⌄</button></div><section class="stats-grid"><div class="stat-card"><span class="stat-label">Net sales</span><strong class="stat-value">$24,680</strong><span class="stat-note">↑ 12.8% vs last month</span><span class="stat-icon bg-mint">↗</span></div><div class="stat-card"><span class="stat-label">Outstanding</span><strong class="stat-value">$3,420</strong><span class="stat-note down">↓ 4.2% vs last month</span><span class="stat-icon bg-yellow">$</span></div><div class="stat-card"><span class="stat-label">Items in stock</span><strong class="stat-value">1,248</strong><span class="stat-note">↑ 6.4% vs last month</span><span class="stat-icon bg-blue">▦</span></div><div class="stat-card"><span class="stat-label">Returns this month</span><strong class="stat-value">18</strong><span class="stat-note down">↑ 2 from last month</span><span class="stat-icon bg-coral">↩</span></div></section><div class="dashboard-grid"><section class="panel"><div class="panel-header"><h2>Recent invoices</h2><a data-view-link="invoices">View all →</a></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Invoice</th><th>Customer</th><th>Status</th><th>Total</th></tr></thead><tbody>${invoiceRows()}</tbody></table></div></section><section class="panel"><div class="panel-header"><h2>Activity</h2><a>Today</a></div><div class="activity">${activities.map(a => `<div class="activity-row"><span class="activity-dot ${a[4]}">${a[0]}</span><div class="activity-copy"><b>${a[1]}</b><small>${a[2]}</small></div><span class="activity-amount">${a[3]}</span></div>`).join('')}</div></section></div><div class="quick-actions"><button class="quick-action" data-view-link="inventory"><span>▦</span><b>Adjust stock</b><small>Receive or move items</small></button><button class="quick-action" data-view-link="delivery"><span>⌁</span><b>Delivery note</b><small>Prepare an order</small></button><button class="quick-action" data-view-link="returns"><span>↩</span><b>Process return</b><small>Restock an item</small></button></div>`; }
